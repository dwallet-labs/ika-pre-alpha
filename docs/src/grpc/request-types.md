...
pub enum DWalletRequest {
    ...
    Sign { ... },                           // Defined on-wire, mock returns error (needs on-chain lookup)
    ImportedKeySign { ... },                // Defined on-wire, mock returns error (needs on-chain lookup)
    Presign { ... },                        // Supported in mock
    PresignForDWallet { ... },              // Supported in mock
    ImportedKeyVerification { ... },        // Supported in mock
    ReEncryptShare { ... },                 // Defined on-wire, mock returns error
    MakeSharePublic { ... },                // Defined on-wire, mock returns error
    FutureSign { ... },                     // Defined on-wire, mock returns error
    SignWithPartialUserSig { ... },         // Defined on-wire, mock returns error
    ImportedKeySignWithPartialUserSig { ... }, // Defined on-wire, mock returns error
}
```
...
|---------|-------------|-------|
| `DKG` | Supported | All 4 curves (Secp256k1, Secp256r1, Curve25519, Ristretto). Encrypted or Public mode (mock treats both identically). Auto-commits dWallet on-chain and transfers authority to `intended_chain_sender`. Ristretto DKG uses real Schnorrkel keypairs. |
| `Sign` | Returns error | `signature_scheme` is now derived from the on-chain `MessageApproval`; the mock cannot look up on-chain state, so dispatch returns an error. |
| `ImportedKeySign` | Returns error | Same as Sign -- requires on-chain lookup. |
| `Presign` | Supported | Returns attestation with presign data. Uses `signature_algorithm` (not `signature_scheme`). |
| `PresignForDWallet` | Supported | Same as Presign in mock. Uses `dwallet_public_key` (not `dwallet_id`). |
| `ImportedKeyVerification` | Supported | Creates an imported-key dWallet. Uses `UserSecretKeyShare` (Encrypted or Public). |
| `ReEncryptShare` | Not implemented | Wire format defined; mock returns error. |
| `MakeSharePublic` | Not implemented | Wire format defined; mock returns error. |
| `FutureSign` | Not implemented | Wire format defined; mock returns error. |
| `SignWithPartialUserSig` | Not implemented | Wire format defined; mock returns error. |
| `ImportedKeySignWithPartialUserSig` | Not implemented | Wire format defined; mock returns error. |
