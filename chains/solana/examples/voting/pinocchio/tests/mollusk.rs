...
//! Mollusk instruction-level tests for the ika voting example.
//!
//! Tests create_proposal and cast_vote in isolation, plus the full CPI path
//! where quorum triggers approve_message on the dWallet program.
...
const PROGRAM_PATH: &str = concat!(
    ...
    "/../../../../../target/deploy/ika_example_voting_pinocchio"
);
...
///
/// Data layout: proposal_id(32) + message_hash(32) + user_pubkey(32) +
///              signature_scheme(1) + quorum(4) + message_approval_bump(1) + bump(1) = 103
///
...
fn build_create_proposal_ix(
    ...
    Instruction {
        ...
        accounts: vec![
            AccountMeta::new(*proposal, false),                 // proposal PDA (writable)
            AccountMeta::new_readonly(*dwallet, false),         // dwallet
...
///
/// Data layout: proposal_id(32) + vote(1) + vote_record_bump(1) + cpi_authority_bump(1) = 35
///
/// Accounts (no quorum path): [proposal(w), vote_record(w), voter(s), payer(ws), system_program(r)]
fn build_cast_vote_ix(
    ...
    Instruction {
        ...
        accounts: vec![
            AccountMeta::new(*proposal, false),                 // proposal PDA (writable)
            AccountMeta::new(*vote_record, false),              // vote_record PDA (writable)
            AccountMeta::new_readonly(*voter, true),            // voter (signer)
            AccountMeta::new(*payer, true),                     // payer (writable, signer)
            AccountMeta::new_readonly(SYSTEM_PROGRAM_ID, false), // system program
        ],
        data: ix_data,
...
fn test_create_proposal_success() {
    ...
    // Derive the proposal PDA so the seeds match what the program expects.
...
fn test_create_proposal_already_exists() {
    ...
    // Pre-populate the proposal account with existing data (already created).
...
fn test_cast_vote_yes_success() {
    ...
    // Derive proposal PDA (already created, we provide pre-populated data).
    let (proposal_pda, proposal_bump) =
        Pubkey::find_program_address(&[b"proposal", &proposal_id], &program_id);
    ...
    let proposal_data = build_proposal_data(
        ...
        0,                     // yes_votes
        0,                     // no_votes
        5,                     // quorum (high so quorum is NOT reached)
        STATUS_OPEN,
        proposal_bump,
    );
    ...
    // Check VoteRecord was created correctly.
    ...
    // Check proposal yes_votes incremented.
    let prop_data = &result.resulting_accounts[0].1.data;
...
fn test_cast_vote_no_success() {
    ...
    let proposal_data = build_proposal_data(
        ...
        0,
        0,
        5, // quorum high enough to not trigger CPI
        STATUS_OPEN,
        proposal_bump,
    );
    ...
    // Check proposal no_votes incremented, yes_votes unchanged.
    let prop_data = &result.resulting_accounts[0].1.data;
    assert_eq!(read_u32(prop_data, PROP_YES_VOTES), 0, "yes_votes = 0");
...
fn test_cast_vote_closed_proposal_fails() {
    ...
    // Proposal is already approved (closed).
    let proposal_data = build_proposal_data(
        &proposal_id,
        &dwallet_key,
        &[0x42u8; 32],
